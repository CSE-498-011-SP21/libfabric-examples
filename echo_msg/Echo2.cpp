...
int start_server() {
    ...
    */
    ...
    if (dst_addr) {
        /* client */
        safe_call(fi_getinfo(FI_VERSION(1, 6), dst_addr, port, 0, hints, &fi_pep));
    } else {
        /* server */
        safe_call(fi_getinfo(FI_VERSION(1, 6), NULL, port, FI_SOURCE, hints, &fi_pep));
    }
    ...
    std::cout <<"Event queue fid: " << &eq->fid << std::endl;
    std::cout <<"Passive ep: " << pep << std::endl;
...
int server_connect(void) {
    ...
    */
    ...
    fi_domain_bind(domain, &eq->fid, 0);
...
int open_fabric_res(void) {
	...
	safe_call(fi_domain_bind(domain, &eq->fid, 0));
...
int client_connect(void) {
    ...
    */
    ...
    // getinfo 127.0. 0.1
    if (!dst_addr) {
    	std::cout << "No destination address found" << std::endl;
    }
    ...
    // Allocate resources
    safe_call(alloc_active_res(fi));

    //FT_EP_BIND(ep, eq, 0);

    safe_call(fi_enable(ep));

    std::cout << fi->dest_addr << std::endl;

    safe_call(connect_ep(ep, eq, fi->dest_addr));
    safe_call(fi_connect(ep, remote_addr, NULL, 0));
